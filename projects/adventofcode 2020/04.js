...
let i = 0;
input_raw.forEach((e) => {
  let byr = 1920 <= Number(e.byr) && Number(e.byr) <= 2002;
  let iyr = 2010 <= Number(e.iyr) && Number(e.iyr) <= 2020;
  let eyr = 2020 <= Number(e.eyr) && Number(e.eyr) <= 2030;

  let hgt = false;
  if (e.hgt != undefined) {
    let hgt_tmp = {
      value: e.hgt.substring(0, e.hgt.length - 2),
      unit: e.hgt.substring(e.hgt.length - 2),
    };
    switch (hgt_tmp.unit) {
      case 'cm':
        hgt = 150 <= Number(hgt_tmp.value) && Number(hgt_tmp.value) <= 193;
        break;
      case 'in':
        hgt = 59 <= Number(hgt_tmp.value) && Number(hgt_tmp.value) <= 76;
        break;
    }
  }

  let hcl = e.hcl != undefined && e.hcl.length == 7 ? /^#[a-f0-9]+$/.test(e.hcl) : false;
  let ecl = ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'].includes(e.ecl);
  let pid = e.pid != undefined && e.pid.length == 9 ? /^[0-9]+$/.test(e.pid) : false;
