...
function clearInput(input) {
  ...
  input.forEach((e) => {
    ...
    answer.push({ name: name, next: next });
  });
  return answer;
}

let input_test = clearInput(input_test_raw);

function howManyBagsContain(input, target) {
  let count = 0;
  input.forEach((rule) => {
    let tmp = isTheNextOneTarget(rule, target, input);
    if (tmp) {
      count++;
    }
  });
...
function isTheNextOneTarget(obj, target, objs) {
  //console.log(obj);
  if (obj.next.includes(target)) {
    return true;
  } else {
    let containsTarget = [];
    obj.next.forEach((e) => {
      let nextOnes = objs.filter((bag) => {
        return bag.name === e;
      });

      nextOnes.forEach((element) => {
        containsTarget.push(isTheNextOneTarget(element, target, objs));
      });
    });
    return containsTarget.includes(true);
  }
}*/
/*
let input_final = clearInput(input_raw);
console.log(howManyBagsContain(input_final, 'shiny gold'));*/
/*
...
function clearInputv2(input) {
  ...
  input.forEach((e) => {
    ...
    for (let i = 0; i < arr.length; i += 4) {
      let tmp = arr[i + 1] + ' ' + arr[i + 2];
      if (tmp != 'other bags.') {
        next.push({ name: tmp, value: arr[i] });
      }
    }
    answer.push({ name: name, next: next });
  });
...
function findByName(target, input) {
  let answer = input.filter((bag) => {
    return bag.name === target;
  })[0];
  return answer;
}

function howManyBagsContains(target, input) {
  input = clearInputv2(input);
  let head = findByName(target, input);
  let aw = track(head, input) - 1;
  return aw;
}

function track(node, input) {
  let answer = 1;
  if (node.next.length > 0) {
    node.next.forEach((next) => {
      answer += next.value * track(findByName(next.name, input), input);
    });
  } else {
    return 1;
  }
...
console.log(howManyBagsContains('shiny gold', input_raw));
