...
function numberOfAnswersAnyoneAnswered(input) {
  input = input.split('\n\n');
  let sum = 0;
  var uniq = '';
...
function numberOfAnswersEveryoneAnswered(input) {
  input = input.split('\n\n');
  var uniq = '';
  let count = 0;
  for (let i = 0; i < input.length; i++) {
    ...
    uniq.split('').forEach((e) => {
      let re = new RegExp('[^' + e + ']', 'g');

      if (input[i].replace(re, '').length == people) {
        count++;
      }
    });
  }
...
console.log(numberOfAnswersAnyoneAnswered(input_raw));
console.log(numberOfAnswersEveryoneAnswered(input_raw));
