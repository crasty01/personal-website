...
//-------RANDOM OBRÁZKY-------//
let list = [
    'motyl', 'pes', 'kocka', 'beruska', 'koruna', 'prsten', 'clovek', 'zirafa', 'blb', 'buh',
    'osel', 'skola', 'ubrousek', 'traktor', 'uzMěNicNenapadá', 'nehoda', 'pírko', 'kolébka', 'oblečení', 'dárek',
    'vodník', 'kolekce', 'tablet', 'lusk', 'andulka', 'prak', 'oslava', 'vražda', 'slabost', 'zločin',
    'moped', 'kurz', 'koberec', 'hnědooká', 'komín', 'chlup', 'brk', 'sešit', 'malování', 'banka',
    'dlaň', 'čtverec', 'vzor', 'meč', 'pásmo', 'kmín', 'parfém', 'skákání', 'skořápka', 'rakev'
];
...
function setRandomCards() {
    for (blank; blank.length > 0; blank) {
        ...
        for (i; i < 2; i++) {
            ...
            blank[random].dataset.card = list[numOfReptes];
...
//------ADMIN'S TOOLS------//
function odhalVseKromneJedneKarty() {
    for (let i = 0; i < karty.length; i++) {
        if (i > 0) {
            karty[i].classList.remove('karta');
            karty[i].classList.add('karta-out');
        }
    }
};